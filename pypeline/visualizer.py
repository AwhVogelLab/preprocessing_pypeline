...
class Visualizer:
    ...
    def preprocess_data_for_plot(self, downscale=None, chan_offset=None):
        ...
        self.offset_dict_stacked = {
            ...
            "misc": -self.ys[self.chan_types == "misc"].mean(),  # StimTrak, etc
        }
        ...
        self.ylim_stacked = (
            np.nanpercentile(self.epochs_stacked[:, -1], 5) - chan_offset * 1.5,  # 5th percentile of lowest channel + 2 chans
    ...
    def plot_pos(self, pos):
        ...
        if self.rejection_time[0] == self.trial_start and self.rejection_time[1] == self.trial_end:
            ...
            self.ax.set_xticks(np.arange((self.rejection_time[1] - self.trial_start) * self.srate, self.epoch_len * self.win_step, self.epoch_len))
            self.ax.set_xticklabels([int(self.rejection_time[1] * 1000)] * self.win_step)

        elif self.rejection_time[1] == self.trial_end:
            self.ax.set_xticks(np.arange((self.rejection_time[0] - self.trial_start) * self.srate, self.epoch_len * self.win_step, self.epoch_len))
